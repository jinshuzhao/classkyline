var waitSrc = "/Content/image/loading.gif";
var JqueryDialog = {
    ...
    //Header背景色
    "cHeaderBackgroundColor"	:	"#2374c6",      //bfbfbf
    //右上角关闭显示文本
    "cCloseText"				:	"<i class='fa fa-times fa-2x'></i>",
    //鼠标移上去时的提示文字
    "cCloseTitle"				:	"Close",
    ...
    Open: function (dialogTitle, iframeSrc, iframeWidth, iframeHeight,isSubmitButton, isCancelButton, isDrag,cAlert) {

        JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,cAlert);
    },
    ...
    Open2:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
        JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, false, true, true);
    },
    ...
    WaitEvent: function (imgsrc) {
        ...
        _top = (document.documentElement.clientHeight - (_iframeHeight +50)) / 2;
	...
	/// <param name="isCancelButton">是否呈现“取消”按钮</param>
	/// <param name="isDrag">是否支持拖拽</param>
    init: function (dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,cAlert) {

        document.body.setAttribute('hidOverflowStyle', document.body.style.overflow);
		...
		_top = (document.documentElement.clientHeight - (iframeHeight + JqueryDialog.cBorderSize * 2 +100)) / 2;
		var _top_sr = $("body").scrollTop();
		if (_top_sr == 0) _top_sr = document.documentElement.scrollTop;
		...
		_jd_dialog_m_h.append("<span id='jd_dialog_m_h_r' title='" + JqueryDialog.cCloseTitle + "' onclick='JqueryDialog.Close("+cAlert+");'>" + JqueryDialog.cCloseText + "</span>");
		...
		//iframe
		$("#jd_dialog_m_b_2").append("<iframe id='jd_iframe'  name='js_iframe' src='" + iframeSrc + "' scrolling='yes' frameborder='0' width='" + iframeWidth + "' height='" + iframeHeight + "' />");
        //增加对应按钮的处理，modify by gxg 20140923
        var str="<div style='text-align:center;margin-top:20px;margin-bottom:20px;' class='form-btn'>"
		if (isSubmitButton) {
		     str += "<a href='#' class='btn blue'  onclick='btnok()'>"+this.cSubmitText+"</a>";

		}
        if (isCancelButton) {
		        str += "<a class='btn grey' href='#' style='margin-left:10px' onclick='btncancel()'>"+this.cCancelText+"</a>";
		    }
        str+="</div>"
	 $("#jd_dialog_m_b_2").append(str);
    ...
    Close: function (cAlert) {
        var con=false;
         if(cAlert){
            con=window.confirm("请确认已经关闭当前vm");
              if(con){
              document.body.style.overflow = document.body.getAttribute('hidOverflowStyle');
        $("#jd_shadow").remove();
        //$("#jd_dialog").remove();
	   $("#jd_dialog").hide();
        }
